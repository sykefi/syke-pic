...
class ModelData():
    ...
    def __init__(self, dataset, split, min_N, max_N,
                 exclude=[], random_seed=24):
    ...
    def _init_paths(self):
        ...
        for class_dir in self.dataset.iterdir():
            paths = sorted(list_files(class_dir, '.png', self.min_N, self.max_N,
                                      self.exclude, self.random_seed))
            if not paths:
                continue
            ...
            self.distribution[class_dir.name] = [len(paths), len(train),
                                                 len(val), len(test)]
    ...
    def set_data_loaders(self, batch_size, num_workers, train_transform,
                         eval_transform, num_chans=3):
        ...
        if self.oversampled:
            ...
            train_x, train_y = combined_shuffle(
                train_x, train_y, self.random_seed)
        else:
            train_x = self.train_x
            train_y = self.train_y

        train_data = ImageDataset(train_x, train_y, self.train_transform,
                                  num_chans, len(self.le.classes_))
        val_data = ImageDataset(self.val_x, self.val_y, self.eval_transform,
                                num_chans, len(self.le.classes_))
        test_data = ImageDataset(self.test_x, self.test_y, self.eval_transform,
                                 num_chans, len(self.le.classes_))
        self.train_loader = DataLoader(train_data, batch_size, shuffle=True,
                                       num_workers=num_workers)
        self.val_loader = DataLoader(val_data, batch_size,
                                     num_workers=num_workers)
        self.test_loader = DataLoader(test_data, batch_size,
                                      num_workers=num_workers)
...
class ImageDataset(Dataset):
    ...
    def __init__(self, paths, labels=None, transform=None, num_chans=3,
                 num_classes=None):
        self.paths = paths
        self.labels = labels
...
def list_files(root_dir, extension, min_N=None, max_N=None,
               exclude=[], random_seed=24):
