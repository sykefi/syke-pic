...
def main(args):
    ...
    exclude = [name.strip() for name in
               config.get('dataset', 'exclude').split(',')]
    random_seed = config.getint('dataset', 'random_seed')
    model_data = data.ModelData(
        dataset, split, min_N, max_N, exclude, random_seed)
    ...
    if args.collage:
        ...
        model_data.set_data_loaders(batch_size, num_workers, train_transform,
                                    eval_transform, num_chans=img_shape[0])
        if not out_file.suffix:
            out_file = out_file.with_suffix('.png')
    ...
    model_dir.mkdir(
        parents=True, exist_ok=config.getboolean('model', 'exist_ok'))
    ...
    # [train]
    device = torch.device(
        'cuda:0' if config.getboolean('train', 'gpu') else 'cpu')
    ...
    if config.getboolean('lr_warmup', 'use'):
        ...
        lr_warmup = network.LRWarmup(
            net, optimizer, f1, f2, s1, s2, s3, verbose)
    else:
        lr_warmup = None
    ...
    # Train and Evaluate
    best_state = train_net(net, model_data.train_loader, model_data.val_loader,
                           optimizer, loss_fn, max_epochs, early_stop_patience,
                           model_dir, device, lr_scheduler, lr_warmup)
    net.load_state_dict(torch.load(best_state))
    test_report = test_net(net, model_data.test_loader,
                           model_data.le.classes_, device)
    print(test_report)
    with open(model_dir/'test_report.txt', 'w') as fh:
        fh.write(test_report)


def train_net(net, train_dataloader, val_dataloader, optimizer,
              loss_fn, max_epochs, early_stop_patience, model_dir,
              device, lr_scheduler=None, lr_warmup=None):
    ...
    try:
        for epoch in range(1, max_epochs + 1):
            ...
            print(f'[STAT] Train Acc: {train_acc:.3f}, '
            ...
            # Checkpoint
            plot.plot_stats(train_accuracies, train_losses, val_accuracies,
                            val_losses, outfile=model_dir/'train_stats.png',
                            first_epoch=1, epoch_step=3)
            if epoch >= 11:
                plot.plot_stats(train_accuracies[10:], train_losses[10:],
                                val_accuracies[10:], val_losses[10:],
                                outfile=model_dir/'train_stats_zoomed.png',
                                first_epoch=11, epoch_step=2)
            ...
            if val_loss < min_val_loss or epoch == 1:
                ...
                no_improvement += 1
                print(
                    f'[INFO] No reduction in loss for {no_improvement} epochs')
