...
from .image import (Compose, Resize, FlipHorizontal, FlipVertical,
                    Translate, Zoom, Rotate, ChangeBrightness)
...
def get_transforms(config, img_shape):
    augmentations = [aug.strip() for aug in config.get(
                     'image', 'augmentations').split(',')]
    ...
    for aug in augmentations:
        ...
        if aug == 'zoom':
            zoom_range = tuple(float(i) for i in config.get(
                'image', 'zoom_range').split(','))
            train_transform.append(Zoom(zoom_range))
        if aug == 'brightness':
            brightness_range = tuple(float(i) for i in config.get(
                'image', 'brightness_range').split(','))
