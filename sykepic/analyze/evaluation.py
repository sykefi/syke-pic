...
def results_as_df(
    ...
    for idx, row in eval_df.iterrows():
        ...
        for threshold in threshold_values:
            ...
            for name, result in classification_result(prediction, actual, empty):
                ...
                result_dict[name].setdefault(
                    threshold, {"tp": 0, "tn": 0, "fp": 0, "fn": 0}
                )[result] += 1
    ...
    result_df = pd.DataFrame.from_dict(result_dict, orient="index").sort_index()
    ...
    if not threshold_search:
        ...
        if empty in result_df.index:
            ...
            result_df = pd.concat([result_df.loc[["all"], :], result_df.drop("all")])
        # Add threshold column
        result_df.insert(
            0,
            "threshold",
            result_df.apply(lambda row: thres_dict.get(row.name, np.nan), axis=1),
        )
    ...
    score_df = result_df.apply(
        lambda row: classification_scores(row.tp, row.tn, row.fp, row.fn),
        axis=1,
        result_type="expand",
    )
