...
class PredictionViewer():
    ...
    def __init__(self, prediction_csv, raw_dir, work_dir='PredictionViewer',
                 label=False, keep_images=False):
        self.work_dir = Path(work_dir)
        # Read csv to pandas DataFrame
        if isinstance(prediction_csv, list):
            assert not label, 'Labeling is allowed one sample a time'
            df_list = []
            # self.sample = []
            self.img_dir = {}
    ...
    def start(self, per_page=12, class_inspect=False, sort_by_confidence=True,
              ascending=False, prediction_filter=None):
        """Start the program
    ...
    def _new_item(self, roi):
        ...
        options = self.extra_labels.copy() if self.label else []
        options.extend(
        ...
        item = Box(children=(img, label, dropdown),
