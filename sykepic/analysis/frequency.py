...
def frequency_df(
    ...
    csv_date_list = filter_csv_by_date(pred_dir, start, end, hour_window, date_format)
...
def filter_csv_by_date(
    pred_dir, start=None, end=None, hour_window=None, date_format="%Y-%m-%d %H:%M"
):
    ...
    if hour_window:
        ...
        hour_start = datetime.datetime.strptime(hour_start.strip(), time_format)
    ...
    for csv in sorted(pred_dir.glob("**/*.csv")):
        ...
        if hour_window and not (hour_start.time() <= date.time() <= hour_end.time()):
            continue
