...
def main(args):
    ...
    for p, v in tqdm(files):
        ...
        df = pd.concat([read_predictions(p, args.thresholds),
                        pd.read_csv(v, index_col=0)], axis=1)
        df.index.name = 'roi'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'predictions', help='Path to directory with predictions')
    parser.add_argument(
        'biovolumes', help='Path to directory with biovolumes')
    parser.add_argument(
        'thresholds', help='File with confidence thresholds')
    parser.add_argument(
        'outdir', help='Directory where results are saved')
    parser.add_argument(
        '-d', '--division', help='File with intra-class divisions')
